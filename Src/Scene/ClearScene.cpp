...
void ClearScene::Init(void)
{
	...
	animationControllerPlayer_->Add(0, path + "PPlayer.mv1", ANIM_SPEED, ANIM_NUM);
	...
	redAlly_.quaRot = Quaternion::Euler(ALLY_ROT, AsoUtility::Deg2RadF(ALLY_INIT_ROT_Y_DEG), 0.0f);
	redAlly_.Update();

	//赤い味方のアニメーション
	std::string path1 = Application::PATH_MODEL + "Ally/";
	animationControllerRed_ = std::make_unique<AnimationController>(redAlly_.modelId);
	animationControllerRed_->Add(0, path1 + "RedAlly.mv1", -ALLY_INIT_ROT_Y_DEG, ANIM_NUM);
	...
	blueAlly_.quaRot = Quaternion::Euler(ALLY_ROT, AsoUtility::Deg2RadF(ALLY_INIT_ROT_Y_DEG), 0.0f);
	blueAlly_.Update();

	//青い味方のアニメーション
	animationControllerBlue_ = std::make_unique<AnimationController>(blueAlly_.modelId);
	animationControllerBlue_->Add(0, path1 + "BuleAlly.mv1", -ALLY_INIT_ROT_Y_DEG, ANIM_NUM);
	...
	blackAlly_.quaRot = Quaternion::Euler(ALLY_ROT, AsoUtility::Deg2RadF(ALLY_INIT_ROT_Y_DEG), 0.0f);
	blackAlly_.Update();

	//黒い味方のアニメーション
	animationControllerBlack_ = std::make_unique<AnimationController>(blackAlly_.modelId);
	animationControllerBlack_->Add(0, path1 + "BlackAlly.mv1", -ALLY_INIT_ROT_Y_DEG, ANIM_NUM);
