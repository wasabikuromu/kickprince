...
InputManager::JOYPAD_IN_STATE InputManager::GetJPadInputState(JOYPAD_NO no)
{
	...
	case InputManager::JOYPAD_TYPE::XBOX_ONE:
	{
		...
		idx = static_cast<int>(JOYPAD_BTN::D_DOWN);
		ret.ButtonsNew[idx] = x.Buttons[1];// 下

		idx = static_cast<int>(JOYPAD_BTN::D_TOP);
		ret.ButtonsNew[idx] = x.Buttons[0];// 上

		idx = static_cast<int>(JOYPAD_BTN::D_LEFT);
		ret.ButtonsNew[idx] = x.Buttons[2];// 左

		idx = static_cast<int>(JOYPAD_BTN::D_RIGHT);
		...
		ret.ButtonsNew[idx] = d.Buttons[7];// メニュー
		...
		ret.ButtonsNew[idx] = d.Buttons[8];// L押し込み

		idx = static_cast<int>(JOYPAD_BTN::R_STICK_PUSH);
		ret.ButtonsNew[idx] = d.Buttons[9];// R押し込み
